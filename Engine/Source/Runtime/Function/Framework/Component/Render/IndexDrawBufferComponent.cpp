...
void IndexDrawBufferComponent::GenerateIndexDrawBuffer(const MeshComponent& mesh_component) {
    const RendererDemo::StaticMeshData& mesh_data = mesh_component.MeshData();

    std::vector<StaticMeshDataVertex> vertices = GenerateVertexData(mesh_component);
    ...
    uint32_t vertex_array_id = OpenGLAPI::CreateVertexArray(vertex_id, index_id, KSiampleVertexLayout);
